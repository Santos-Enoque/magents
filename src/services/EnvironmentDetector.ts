...
export class EnvironmentDetector {
  ...
  public isContainerEnvironment(): boolean {
  ...
  public getCodespacesConfig(): any {
    if (!this.isCodespaces()) {
      return null;
    }
  ...
  public generateClaudeCommand(baseCommand: string = 'claude'): string {
  ...
  public shouldUsePermissionsFlag(): boolean {
    const config = this.getEnvironmentConfig();
    return config.claudeFlags.includes('--dangerously-skip-permissions');
  }

  public getOptimizedDockerConfig(): any {
    const config = this.getEnvironmentConfig();
