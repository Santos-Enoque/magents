...
class TaskRepository {
    constructor() {
        this.db = DatabaseService_1.DatabaseService.getInstance().getDatabase();
    }
    /**
     * Create a new task
     */
    ...
    `);
        stmt.run(task.id, task.task_master_id, task.project_id, task.title, task.description, task.details, task.status, task.priority, task.assigned_to_agent_id, task.parent_task_id, task.subtask_ids, task.dependencies, task.test_strategy, task.test_results, task.estimated_effort, task.actual_effort, task.tags, task.metadata, task.created_at, task.updated_at, task.assigned_at, task.started_at, task.completed_at);
    ...
    findWithFilters(filter) {
        ...
        if (filter.hasSubtasks !== undefined) {
            filtered = filtered.filter(task => filter.hasSubtasks ? (task.subtaskIds?.length || 0) > 0 : (task.subtaskIds?.length || 0) === 0);
        }
        if (filter.hasDependencies !== undefined) {
            filtered = filtered.filter(task => filter.hasDependencies ? (task.dependencies?.length || 0) > 0 : (task.dependencies?.length || 0) === 0);
        }
        ...
        if (filter.includeSubtasks) {
            ...
            // Remove duplicates
            const uniqueTasks = Array.from(new Map(tasksWithSubtasks.map(t => [t.id, t])).values());
            return uniqueTasks;
        }
        return filtered;
    }
    ...
    async update(id, input) {
        ...
        params.push(new Date().toISOString());
        if (updates.length === 1)
            return existing; // Only updated_at changed
        // Add ID to params
        params.push(id);
     ...
     */
    async removeSubtaskFromParent(parentId, subtaskId) {
        const parent = this.findById(parentId);
