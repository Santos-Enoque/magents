import { TmuxService } from '@magents/cli';

// Initialize the TmuxService to connect to tmux sessions
const tmuxService = new TmuxService();

export const tmuxController = {
  async getSessionContent(sessionName: string, windowName?: string, lines: number = 100) {
    try {
  ...
  async getSessionInfo(sessionName: string) {
    try {
      if (!tmuxService.sessionExists(sessionName)) {
        throw new Error(`Tmux session '${sessionName}' not found`);
      }
  ...
  async sendCommand(sessionName: string, command: string, windowName?: string) {
    try {
      if (!tmuxService.sessionExists(sessionName)) {
        throw new Error(`Tmux session '${sessionName}' not found`);
      }
