...
import { STANDARD_COMMANDS } from '@magents/shared/dist/core/commands';
import { DockerAgentManager } from './DockerAgentManager';
import { ConfigManager } from '../config/ConfigManager';

export class CoreIntegrationService {
  private static instance: CoreIntegrationService;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      coreManager.syncBridge.subscribe(this.sessionId, [
        'command.executed',
        'agent.created',
        'agent.updated',
        'agent.deleted',
  ...
  async executeCommand(
    ...
    return coreManager.executeCommand(commandName, options);
  }
  ...
  getActivityLogs(filters?: {
    ...
    return coreManager.activityLogger.getLogs({
      ...filters,
      source: 'CLI',
      sessionId: this.sessionId
    });
  ...
  getSyncConflicts(filters?: {
    ...
    return coreManager.conflictResolver.getConflicts(filters);
  }
  ...
  private registerStandardCommands(): void {
    STANDARD_COMMANDS.forEach((CommandClass: any) => {
      const command = new CommandClass();
      
      // Override execute method to use actual CLI services
      const originalExecute = command.execute.bind(command);
      command.execute = async (options: CommandOptions): Promise<CoreCommandResult> => {
        try {
   ...
   */
  private async executeActualCommand(
    commandName: string, 
      ...
      switch (commandName) {
        ...
        case 'list-agents':
          ...
          const filteredAgents = params.status 
            ? agents.filter((agent: any) => agent.status === params.status)
            : agents;
        ...
        case 'config-get':
          ...
          const value = (config as any)[params.key];
          return {
            success: true,
        ...
        case 'config-set':
          ...
          (currentConfig as any)[params.key] = params.value;
  ...
  private setupEventListeners(): void {
    coreManager.syncBridge.onSync(this.sessionId, (payload: EventPayload) => {
   ...
   */
  private async getSystemStatus(detailed?: boolean): Promise<any> {
    const agents = this.dockerManager.getActiveAgents();
   ...
   */
  private async performCleanup(removeWorktrees?: boolean, force?: boolean): Promise<any> {
    ...
    try {
      ...
      for (const agent of stoppedAgents) {
        try {
          ...
          cleaned.agents++;
          if (removeWorktrees) cleaned.worktrees++;
        } catch (error) {
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      coreManager.syncBridge.unsubscribe(this.sessionId);
      coreManager.syncBridge.offSync(this.sessionId);
