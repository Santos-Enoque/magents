...
echo "=== Quick Test for Magents Docker Runtime ==="
echo ""
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        *)
            ...
            echo "Usage: $0 [--keep]"
            echo "  --keep: Keep the container running after the test"
            exit 1
            ;;
    esac
done
...
# Create a modified bridge server script that uses our socket path
...
if docker exec magents-test curl -sf http://localhost:3999/health > /dev/null 2>&1; then
    ...
    echo "     Checking if health server is running:"
    docker exec magents-test ps aux | grep -E "(health-server|node.*health)" || echo "     Health server not found in process list"
fi
...
echo ""
echo "You can now:"
echo "  • Connect to the container: docker exec -it magents-test bash"
echo "  • Test Claude bridge: docker exec -it magents-test claude --version"
echo "  • Check health status: curl http://localhost:$(docker port magents-test 3999 2>/dev/null | cut -d: -f2)/health"
echo ""
if [ "$KEEP_CONTAINER" = true ]; then
    echo "Container will remain running after this script exits."
    echo "When done testing, run: docker rm -f magents-test"
else
    echo "Container will be removed when this script exits."
...
# Note about bridge server
if [ ! -z "$BRIDGE_PID" ]; then
    echo "Note: Claude bridge server is running in background"
    echo "It will be stopped automatically when this script exits"
fi